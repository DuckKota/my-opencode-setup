# codebase-memory-gate Specification

## Purpose

Defines how the codebase-memory plugin interacts with the agent's search tooling: searches are never blocked, and the graph-aware search reminders are optional, throttled, and only active when the graph is armed. (TBD — refine as the capability evolves.)
